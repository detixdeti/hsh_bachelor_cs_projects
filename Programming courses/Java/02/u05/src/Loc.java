...
public class Loc {
  ...
  private int y;
      ...
      /**
       * returns manhattanDistnace between 2 cities
       * @param city.x >= 0
       * @param city.y >= 0
       * @throws IllegalArgumentException if city.x or city.y < 0
       */
      public int manhattanDistance(Loc city) {
          if (city.x < 0 || city.y < 0) {
              throw new IllegalArgumentException("Koordinaten f�r die Stadt m�ssen positiv sein!");
          }
          double dx = Math.abs(x - city.x);
          double dy = Math.abs(y - city.y);
          return (int) (dx + dy);
      }
